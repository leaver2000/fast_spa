...
cdef double rad2deg(double rad) noexcept nogil: # type: ignore
...
@cython.boundscheck(False)
@cython.boundscheck(False)
cdef double[:] _calculate_true_obliquity_of_the_ecliptic(double[:] jme, double[:] delta_eps):
    cdef int i, n
...
def _fast_spa(
    ...
    # 3.5. Calculate the true obliquity of the ecliptic
    e = _calculate_true_obliquity_of_the_ecliptic(jme, delta_epsilon)
